...
module.exports = (wsx) => {
    wsx.on('connect', (client) => {
        if (!client.query) {
            ...
            execute(null, { auth: true }, { body: { auth } }, client).then((result) => {
                console.log(result);
                if (!result) {
    ...
    wsx.on('disconnect', (client) => {
        console.log(client);
        const userIndex = connectIds.findIndex((v) => v.clientId == client.headers['sec-websocket-key']);
        connectIds.splice(userIndex, 0);
        client.send({ type: 'user:diconnect' });
    });

    wsx.clientMessage({ type: 'game:init' }, GameInitInterface, (client, data) => {});
};
