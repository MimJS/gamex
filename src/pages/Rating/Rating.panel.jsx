...
import { VIEW_HOME, VIEW_RATING } from "../../lib/configs/config.vkui";
...
export const RatingPanel = ({ id }) => {
  ...
  return (
    <Panel id={id}>
        ...
        {isError ? (
          ...
          <List>
            {ratingData.serverData.map((v, i) => {
              console.log(ratingData);
              const userVkData = ratingData.vkData
                ? ratingData.vkData.find((f) => f.id == v.id)
                : null;
              return (
                <RatingCell
                  vkData={userVkData}
      ...
      <MyRating />
      <CustomSnackbar viewName={VIEW_RATING} />
    </Panel>
  );
};
