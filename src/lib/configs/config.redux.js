// type consts

// user.reducer.js:
export const USER_SET_VK_DATA = "user/setVkData";
export const USER_SET_SERVER_DATA = "user/setServerData";
export const USER_SET_VK_TOKEN = "user/setVkToken";
export const USER_SET_VK_TOKEN_RIGHTS = "user/setVkTokenRights";

// ws.reducer.js:
export const WS_INIT = "ws/setWSData";
export const WS_SET_TOKEN = "ws/setWSToken";
export const WS_SET_STATUS = "ws/setStatus";
export const WS_SET_IS_CONNECTED = "ws/setIsConnected";

// game.reducer.js:
export const GAME_SET_NAME = "game/setName";
export const GAME_SET_DATA = "game/setData";
export const GAME_SET_GAMELIST = "game/setGameList";
export const GAME_SET_GAMES_ONLINE = "game/setGamesOnline";
